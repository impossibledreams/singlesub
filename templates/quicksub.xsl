...
#
#   PLEASE NOTE: The code below that is being auto-generated by this stylesheet
#   automatically falls under the GPL since quickSub is licensed under the GPL.
#   HOWEVER, SingleSub itself doesn't fall under the GPL and is licensed as described
#   above. See the following for more details:
#
...
-->
	...
	// --------------------------------
	// The section appearing below has been automatically generated by SingleSub and is intended
	// to be used with Jason Brome's QuickSub function (http://www.methodize.org/quicksub/).
	//
	...
	//
	// Please note that this auto generated section automatically falls under the GPL.
	//
	...
	var txtVersion = "0.3.5 [<xsl:value-of select="$version"/>]";
...
clearTimeout(oTimer);
	...
	<xsl:template match="outline[@type='subscribe']">
		"<xsl:value-of select='@text'/>", "<xsl:value-of select='@subscribeUrl'/>", <xsl:choose>
		<xsl:when test="@escaped = 'true'">ESCAPED</xsl:when>
		<xsl:otherwise>UNESCAPED</xsl:otherwise>
